...
def run(valPath):
    ...
    env_above_best = []
    for path in paths:
        with open(os.path.join(os.getcwd(), 'baseline_no_MM', '{}_spoofer'.format(valPath), '{}/graphs'.format(path), 'env_above_best.txt'), 'r') as file:
            a = []
            data = file.read()
    ...
    plt.legend(ncols=3)
    plt.savefig(os.getcwd() + "/new_data_compil/bl_no_mm/{}_spoofer".format(valPath) + "/rounded_env_above_best.png")
...
def run_below(valPath):
    ...
    env_below_best = []
    for path in paths:
        with open(os.path.join(os.getcwd(), 'baseline_no_MM/{}_spoofer'.format(valPath), '{}/graphs'.format(path), "env_below_best.txt"), 'r') as file:
            a = []
            data = file.read()
            data = data.replace('\n', " ").replace("[", " ").replace("]", " ").replace(",", "")
            data = data.split()[3:]
    ...
    plt.legend(ncols=3)
    plt.savefig(os.getcwd() + "/new_data_compil/bl_no_mm/{}_spoofer".format(valPath) + "/rounded_env_below_best.png")
