...
def run():
    ...
    for ind,i in enumerate(avg_spread):
        plt.bar(ind, i, yerr=avg_spread_std[ind]/np.sqrt(10000), capsize=1, error_kw={'lw':1}, color=VALS[0])
    ...
    plt.tight_layout()
    plt.savefig(os.getcwd() + "/new_data_compil/no_spoof_2" + "/spread.pdf")
