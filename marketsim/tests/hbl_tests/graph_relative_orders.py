...
def run(valPath):
    ...
    for path in paths:
        with open(os.path.join(os.getcwd(), 'official_rl_1optimal/{}_spoofer'.format(valPath), '{}/graphs'.format(path), 'env_sell_above_best.txt'), 'r') as file:
            a = []
            data = file.read()
    ...
    plt.legend(ncols=3)
    plt.savefig(os.getcwd() + "/new_data_compil/rl_opt/{}_spoofer".format(valPath) + "/env_above_est.pdf")
...
#     for path in paths:
#         with open(os.path.join(os.getcwd(), 'tuned_optimal/{}_spoofer'.format(valPath), '{}/graphs'.format(path), "env_buy_below_est.txt"), 'r') as file:
#             a = []
#             data = file.read()
#             data = data.replace('\n', " ").replace("[", " ").replace("]", " ").replace(",", "")
#             data = data.split()[:-1]
...
#     plt.legend(ncols=3)
#     plt.savefig(os.getcwd() + "/new_data_compil/new_tuned_optimal/{}_spoofer".format(valPath) + "/env_below_est.png")
